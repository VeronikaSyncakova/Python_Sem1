...
def identify_remove_pairs(hand):
    ...
    and if found, removes the pairs from the hand and returns to the lists: the hand with the pairs removed
    and a list of the removed cards"""
    values=[]
    thirds=set()
    for card in hand:
    ...
    c=Counter(values)
    pairs=[]
    for k,v in c.items(): #k is the key part and v is the value
        if v in [2,4]:
            pairs.append(k)
        elif v==3:
            pairs.append(k)
